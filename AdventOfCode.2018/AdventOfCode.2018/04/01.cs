...
namespace AdventOfCode._2018._04
{
    class _01
    {
        public string Main()
        {
            var input = @"[1518-09-28 00:56] wakes up
...
[1518-07-26 00:51] falls asleep".Split('\n');
            ...
            lines = lines.OrderBy(l => l.DateTime).ToList();
            var guards= new Dictionary<int, Guard>();
            int currentGuard = -1;
            var beginRegex = new Regex(@"Guard #(\d+) begins shift");
            var fallsAsleep= "falls asleep";
            var wakesUp = "wakes up";
            foreach (var line in lines)
            {
                if (beginRegex.IsMatch(line.Value))
                {
                    currentGuard = int.Parse(beginRegex.Match(line.Value).Groups[1].Value);
                    if (!guards.ContainsKey(currentGuard))
                    {
                        guards.Add(currentGuard, new Guard(currentGuard));
                    }
                }
                else if (line.Value == fallsAsleep)
                {
                    guards[currentGuard].FallsAsleep(line.DateTime);
                }
                else if (line.Value == wakesUp)
                {
                    guards[currentGuard].WakesUp(line.DateTime);
                }
            }
            var maxAsleep = guards.Max(g => g.Value.SecondsAsleep);
            var guardMostAsleep = guards.First(g => g.Value.SecondsAsleep == maxAsleep);
            return ((guardMostAsleep.Value.LongestSleepPeriod / 60) * guardMostAsleep.Key).ToString();
        }
    }
    ...
    class Line
    {
        ...
        {
            var regex = new Regex(@"\[(?<DateTime>.+)\] (?<Value>.+)");
    ...
    class Guard
    {
        private DateTime? LastAsleep;
        private List<Tuple<DateTime, DateTime>> sleepPeriods = new List<Tuple<DateTime, DateTime>>();
        ...
        public double SecondsAsleep
        {
            get
            {
                return sleepPeriods.Sum(p => (p.Item2 - p.Item1).TotalSeconds);
            }
        }

        public double LongestSleepPeriod
        {
            get
            {
                return sleepPeriods.Max(p => (p.Item2 - p.Item1).TotalSeconds);
            }
        }

        public void WakesUp(DateTime value)
        {
            if (LastAsleep.HasValue)
            {
                if (LastAsleep.Value.Date != value.Date)
                {
                    sleepPeriods.Add(new Tuple<DateTime, DateTime>(LastAsleep.Value, LastAsleep.Value.Date.AddHours(1)));
                    sleepPeriods.Add(new Tuple<DateTime, DateTime>(value.Date, value));
                }
                else
                {
                    sleepPeriods.Add(new Tuple<DateTime, DateTime>(LastAsleep.Value, value));
                }
                LastAsleep = null;
            }
            else
        ...
        public void FallsAsleep(DateTime value)
        {
            LastAsleep = value;
        }
